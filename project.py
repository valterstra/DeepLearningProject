...
def prepare_data_loaders(batch_size=100, val_size=1000):
    ...
    full_train = datasets.CIFAR10(root='.', train=True, download=True, transform=base_transform)
    test_set   = datasets.CIFAR10(root='.', train=False, download=True, transform=base_transform)

    total_train = len(full_train)
    train_size = total_train - val_size
    train_set, val_set = torch.utils.data.random_split(full_train, [train_size, val_size])

    stat_loader = DataLoader(train_set, batch_size=500, shuffle=False)
    ...
    train_set.dataset.transform = transforms.Compose([transforms.ToTensor(), normalize])
    val_set.dataset.transform = transforms.Compose([transforms.ToTensor(), normalize])
    test_set.transform = transforms.Compose([transforms.ToTensor(), normalize])
...
def main():
    ...
    if arch == 0:
        model = ConvNetBaseline(f=f, nf=nf, nh=nh).to(device)
